...
function Loader({
  ...
  useEffect(() => {
    sigma.setSetting("nodeReducer", (id, attrs) => {
      ...
      // hollow, so the two are never taken for one another; which one you are
      // looking at is said in words on the page itself, not in the node's
      // title — the title belongs to the page, not to where it lives.
      const base = attrs.wetopiaPrivate ? { ...attrs, color: hollow } : attrs;
      return id === selected
